...
class ChatService: ObservableObject {
    ...
    func patchChatRoom(chatRoomId: Int, completion: @escaping (Result<String, AFError>) -> Void) {
            let url = Endpoint.chatPatch(chatRoomId: String(chatRoomId)).url
            ...
            APIService.shared
                ...
                .validate()
                .responseDecodable(of: ApiResponse<String>.self, decoder: decoder) { response in
                    switch response.result {
                    case .success(let apiResponse):
                        if let data = apiResponse.data {
                            completion(.success(data))
